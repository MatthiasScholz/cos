...
# Every client can get requests for the automatically assigned ports for each nomad job,
# for example fabio will use consul to figure out the port mapping and direct requests directly to this ports.
resource "aws_security_group" "sg_content_connector" {
  vpc_id      = "${var.vpc_id}"
  name        = "MNG-${var.stack_name}-${var.aws_region}-${var.env_name}-SG-content-connector"
...
# INGRESS
resource "aws_security_group_rule" "sgr_content_connector_ig_fabio_health" {
  type              = "ingress"
  ...
  security_group_id = "${aws_security_group.sg_content_connector.id}"
}

resource "aws_security_group_rule" "sgr_content_connector_ig_fabio" {
  type              = "ingress"
...
resource "aws_security_group_rule" "sgr_content_connector_ig_docker" {
  type              = "ingress"
...
resource "aws_security_group_rule" "sgr_content_connector_eg_all" {
  type      = "egress"
  from_port = 0
  to_port   = 65535
  protocol  = "tcp"
